...
export const LeafletZoneSelector = forwardRef<LeafletZoneSelectorRef, LeafletZoneSelectorProps>(
  (
    {
    ...
    const { selectedZones, hoveredZone, selectZone, deselectZone, clearSelection, isZoneSelected, setHoveredZone } =
      useZoneSelection({
    ...
    const getTileUrl = () => {
      ...
      const providerName =
        tileProvider || (theme === "dark" ? "cartoDBDark" : theme === "light" ? "cartoDB" : "openstreetmap");
    ...
    return <ErrorBoundary onError={onError}>{content}</ErrorBoundary>;
  }
);
